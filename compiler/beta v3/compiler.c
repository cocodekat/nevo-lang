    ...
    while (fgets(rawline, sizeof(rawline), fin)) {
        ...
        if (strncmp(line, "print(", 6) == 0 && line[strlen(line)-1] == ')') {
            ...
            if (arg[0] == '"' && arg[strlen(arg)-1] == '"') {
                ...
                fprintf(fout, "    mov x1, sp          // pointer to buffer\n");
                fprintf(fout, "    mov w2, #0          // digit count\n");
                fprintf(fout, "    mov w4, #10         // divisor\n");

                // convert number in w0 -> ASCII string in buffer (reverse)
                fprintf(fout,
                    "1:  udiv w3, w0, w4        // w3 = w0 / 10\n"
                    "    msub w5, w3, w4, w0    // remainder = w0 - w3*10\n"
                    "    add w5, w5, #'0'\n"
                    "    strb w5, [x1, w2, uxtw]\n"
                    "    add w2, w2, #1\n"
                    "    mov w0, w3\n"
