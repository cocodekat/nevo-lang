...
int main(int argc, char *argv[]) {
    ...
    for (int file_idx = 1; file_idx < argc; file_idx++) {
        ...
        #if defined(_WIN32) || defined(_WIN64)
            snprintf(cmd, sizeof(cmd), "%s %s -o %.*s", compiler, c_file, (int)(dot - output_file), output_file);
        #else 
            snprintf(cmd, sizeof(cmd), "%s %s -o %.*s > /dev/null 2>&1",compiler, c_file, (int)(dot - output_file), output_file);
        #endif
